...
export default function Exercise2Page() {
  ...
  const handleSubmit = () => {
    ...
    genderPairs.forEach(pair => {
      if (userAnswers[pair.id]?.toLowerCase().trim() === pair.feminine.toLowerCase()) {
        correct++;
      }
    });
    setScore(correct);
  ...
  const isCorrect = (id: string) => {
    return userAnswers[id]?.toLowerCase().trim() === genderPairs.find(p => p.id === id)?.feminine.toLowerCase();
  };

  const isCurrentlyCorrect = (id: string) => {
    const answer = userAnswers[id]?.toLowerCase().trim();
    const correct = genderPairs.find(p => p.id === id)?.feminine.toLowerCase();
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-gray-900 p-4 md:p-8 pb-40 mt-20">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-6 md:mb-8">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-800 dark:text-white mb-2">Exercise 2: Masculine to Feminine Conversion</h1>
          <p className="text-base md:text-lg text-gray-600 dark:text-gray-300">Convert each masculine noun to its feminine form</p>
        </div>
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 border-2 border-blue-200 dark:border-blue-700 rounded-lg p-4 mb-6">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="font-semibold text-blue-800 dark:text-blue-400 mb-1">Exercise Description</h3>
              <p className="text-sm text-blue-700 dark:text-blue-300">
                In this exercise, you will convert <strong>masculine nouns to feminine nouns</strong>. For example, if you see "actor", you should type "actress". This includes common gender conversions you'll use in everyday English.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-3">
            {genderPairs.map(pair => {
              ...
              return (
                <div
                  key={pair.id}
                  className={`p-4 rounded-lg border-2 sticky top-4 ${
                    submitted
                      ? correct
                ...
                >
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex-1">
                      <div className="font-semibold text-gray-800 dark:text-white mb-1">{pair.masculine}</div>
                      <div className="text-sm text-gray-500 dark:text-gray-400">→ Feminine form</div>
                    </div>
                    <div className="flex-1">
                      <input
                        type="text"
                        ...
                        disabled={submitted}
                        placeholder="Type feminine form..."
        ...
        {!submitted && (
          <div className="mt-6 bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4">
            ...
            <ol className="list-decimal list-inside text-blue-700 dark:text-blue-300 space-y-1">
              <li>Type the feminine form of each masculine noun</li>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-800 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] dark:shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.3)] p-4 z-50 border-t-2 border-gray-200 dark:border-gray-700">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-lg md:text-xl font-bold text-gray-800 dark:text-white mb-3">✅ Masculine → Feminine Rules</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2 md:gap-4">
            <div className="flex items-start gap-2">
              <span className="text-blue-600 dark:text-blue-400 font-semibold text-sm">-or → -ress:</span>
              <span className="text-gray-700 dark:text-gray-300 text-sm">actor → actress</span>
            </div>
            <div className="flex items-start gap-2">
              <span className="text-pink-600 dark:text-pink-400 font-semibold text-sm">-er → -ess:</span>
              <span className="text-gray-700 dark:text-gray-300 text-sm">waiter → waitress</span>
            </div>
            <div className="flex items-start gap-2">
              <span className="text-purple-600 dark:text-purple-400 font-semibold text-sm">Common:</span>
              <span className="text-gray-700 dark:text-gray-300 text-sm">man → woman, king → queen</span>
            </div>
          </div>
        </div>
      </div>
