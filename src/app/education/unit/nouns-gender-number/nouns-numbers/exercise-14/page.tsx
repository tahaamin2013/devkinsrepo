...
const sections: Section[] = [
  {
    ...
    label: "A. Singular",
    icon: "🔹",
    instruction: "Write the singular of each of the following.",
    answerLabel: "Singular",
    words: [
      { given: "mice", answer: "mouse" },
      { given: "flies", answer: "fly" },
      { given: "watches", answer: "watch" },
      { given: "children", answer: "child" },
      { given: "houses", answer: "house" },
      { given: "cities", answer: "city" },
      { given: "matches", answer: "match" },
      { given: "branches", answer: "branch" },
    ],
  },
  {
    id: "plural",
    label: "B. Plural",
    icon: "🔸",
    instruction: "Write the plural of each of the following.",
    answerLabel: "Plural",
    words: [
      { given: "baby", answer: "babies" },
      { given: "branch", answer: "branches" },
      { given: "bush", answer: "bushes" },
      { given: "wolf", answer: "wolves" },
      { given: "army", answer: "armies" },
      { given: "loaf", answer: "loaves" },
      { given: "goose", answer: "geese" },
      { given: "face", answer: "faces" },
      { given: "wife", answer: "wives" },
      { given: "child", answer: "children" },
      { given: "fox", answer: "foxes" },
      { given: "buffalo", answer: "buffaloes" },
      { given: "potato", answer: "potatoes" },
    ],
  },
];
...
export default function Exercise2Page() {
  ...
  const [activeSection, setActiveSection] = useState<string>("singular");
  const [animKey, setAnimKey] = useState<number>(0);
  const [answers, setAnswers] = useState<Record<string, string>>(() =>
    Object.fromEntries(sections.flatMap((s) => s.words.map((r) => [`${s.id}-${r.given}`, ""])))
  );
  ...
  const correctCount = rows.reduce((acc, r) => {
    const key = `${active.id}-${r.given}`;
    if (getState(answers[key], r.answer) === "correct") acc++;
    return acc;
  }, 0);
        ...
        {allCorrect && (
          ...
          >
            ...
            <p className="text-sm font-semibold text-green-700 dark:text-green-300">
              All correct! You have completed {active.label}.
            </p>
          </div>
        )}
        ...
        >
          ...
          <div className="bg-[#1C1917] dark:bg-white/8 px-6 py-5 flex items-center gap-4 border-b border-white/10">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-gray-400 text-sm mt-0.5">
                Type your answer for each word below
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 bg-gray-50 dark:bg-white/[0.03] border-b border-gray-200 dark:border-white/8">
            <div className="px-4 py-2.5 text-[11px] font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500 flex items-center gap-1.5">
              <span className="w-4 h-4 rounded-full bg-[#1C1917] dark:bg-white flex items-center justify-center text-white dark:text-[#1C1917] text-[9px] font-black">
                {active.id === "singular" ? "P" : "S"}
              </span>
              {active.id === "singular" ? "Plural (given)" : "Singular (given)"}
            </div>
            <div className="px-4 py-2.5 text-[11px] font-bold uppercase tracking-widest text-indigo-500 dark:text-indigo-400 border-l border-gray-200 dark:border-white/8 flex items-center gap-1.5">
              <span className="w-4 h-4 rounded-full bg-indigo-100 dark:bg-indigo-900/40 flex items-center justify-center text-indigo-700 dark:text-indigo-300 text-[9px] font-black">
                {active.id === "singular" ? "S" : "P"}
              </span>
              {active.answerLabel} (type here)
            </div>
          </div>

          {/* Rows */}
          <div className="divide-y divide-gray-100 dark:divide-white/6 bg-white dark:bg-[#1C1917]">
            {rows.map(({ given, answer }) => {
              const key = `${active.id}-${given}`;
              const val = answers[key] ?? "";
              const state = getState(val, answer);
          ...
          <div className="px-5 py-3.5 bg-gray-50 dark:bg-white/[0.02] border-t border-gray-200 dark:border-white/8 flex items-center justify-between gap-4 flex-wrap">
            <p className="text-xs text-gray-500 leading-relaxed">
              ...
              {active.id === "singular"
                ? "Watch for irregular forms — e.g. mice → mouse, children → child, geese → goose."
                : "Watch for spelling changes — e.g. baby → babies, wolf → wolves, potato → potatoes."}
            </p>
            <button
              onClick={() =>
                setAnswers(
