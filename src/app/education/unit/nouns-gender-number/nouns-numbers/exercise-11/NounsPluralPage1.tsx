...
const nouns: NounRow[] = [
  { plural: "Airs",      meaning: "affected manners." },
  { plural: "Goods",     meaning: "merchandise." },
  { plural: "Compasses", meaning: "an instrument for drawing circles." },
  { plural: "Respects",  meaning: "compliments." },
  { plural: "Physics",   meaning: "natural science." },
  { plural: "Irons",     meaning: "fetters." },
  { plural: "Forces",    meaning: "troops." },
];
...
export default function NounsPluralPage1() {
  ...
  function setAnswer(plural: string, value: string) {
    setAnswers((prev) => ({ ...prev, [plural]: value }));
  }

  const totalFields = nouns.length;
  const correctCount = nouns.reduce((acc, n) => {
    if (getState(answers[n.plural]) === "correct") acc++;
    return acc;
  }, 0);
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        ...
        {allCorrect && (
          ...
          >
            ...
            <p className="text-sm font-semibold text-green-700 dark:text-green-300">
              All correct! You have mastered all plural nouns with different meanings.
            </p>
          </div>
        )}
        ...
        >
          ...
          <div className="grid grid-cols-2 bg-gray-50 dark:bg-white/[0.03] border-b border-gray-200 dark:border-white/8">
            <div className="px-4 py-2.5 text-[11px] font-bold uppercase tracking-widest text-indigo-500 dark:text-indigo-400 flex items-center gap-1.5">
          ...
          <div className="divide-y divide-gray-100 dark:divide-white/6 bg-white dark:bg-[#1C1917]">
            {nouns.map(({ plural, meaning }) => {
              const val = answers[plural];
              const state = getState(val);
              const opts = getDropdownOptions(plural);
                ...
                >
                  ...
                  <div className="px-4 py-4 flex items-center gap-2">
                    ...
                    <div className="relative flex-1">
                      <select
                        value={val}
                        onChange={(e) => setAnswer(plural, e.target.value)}
                        className={`
          ...
          <div className="px-5 py-3.5 bg-gray-50 dark:bg-white/[0.02] border-t border-gray-200 dark:border-white/8 flex items-center justify-between gap-4 flex-wrap">
            <p className="text-xs text-gray-500 leading-relaxed">
              ...
              These nouns have{" "}
              <span className="font-semibold text-gray-800 dark:text-gray-200">no singular</span>{" "}
              in this particular sense.
            </p>
            <button
              onClick={() =>
                setAnswers(Object.fromEntries(nouns.map((n) => [n.plural, ""])))
              }
